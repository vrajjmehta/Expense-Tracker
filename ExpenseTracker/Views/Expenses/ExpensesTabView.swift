    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                ...
                Divider()
                LogListView(predicate: ExpenseLog.predicate(with: Array(selectedCategories), searchText: searchText), sortDescriptor: ExpenseSort(sortType: sortType, sortOrder: sortOrder).sortDescriptor)
            }
